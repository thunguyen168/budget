...
export function seedDatabase(db: Database.Database): void {
  const alreadySeeded = (db.prepare('SELECT COUNT(*) as n FROM categories').get() as { n: number }).n > 0
  ...
  const fixedCats: [string, string, number][] = [
    ['Mortgage',            '#1d4ed8', 1],
  ...
  const varCats: [string, string, number][] = [
    ['Transport',           '#0891b2', 12],
    ['Groceries',           '#16a34a', 13],
  ...
  db.transaction(() => {
    for (const [name, colour, order] of fixedCats) {
      const r = insertCat.run(name, colour, 1, order) as Database.RunResult
      catIds[name] = r.lastInsertRowid as number
    }
    for (const [name, colour, order] of varCats) {
      const r = insertCat.run(name, colour, 0, order) as Database.RunResult
      catIds[name] = r.lastInsertRowid as number
    }
  })()
